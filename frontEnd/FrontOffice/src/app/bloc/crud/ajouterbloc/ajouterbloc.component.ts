...
export class AjouterblocComponent implements OnInit {
  ...
  loadFoyers(): void {
    this.http.get<foyer[]>('http://localhost:8089/foyer/retrievefoyer').subscribe({
  ...
  ajouterbloc(): void {
    // Ensure a valid foyer is selected before submitting
    if (!this.bloc.foyer || !this.bloc.foyer.idfoyer) {
      alert('Veuillez sélectionner un foyer valide.');
      return;
    }
  
    this.http.post('http://localhost:8089/bloc/addbloc', this.bloc).subscribe({
      next: () => {
        alert('Bloc ajouté avec succès!');
        this.bloc = { nomBloc: '', capaciteBloc: 0, foyer: {} as foyer }; // Clear the form
      },
      error: err => {
